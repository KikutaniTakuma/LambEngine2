...
namespace Lamb {
	...
	void Gamepad::Input() {
		...
		// 接続状況の確認
		isPadConnecting_ = (dresult == ERROR_SUCCESS);
	}
	...
	bool Gamepad::GetButton(Button type) const {
//#ifdef USE_DEBUG_CODE
...
//#endif // USE_DEBUG_CODE
	...
	bool Gamepad::GetPreButton(Button type) const {
//#ifdef USE_DEBUG_CODE
//		if (ImGui::GetIO().WantCaptureMouse or ImGui::IsAnyItemHovered()) {
//			return false;
//		}
//#endif // USE_DEBUG_CODE
	...
	bool Gamepad::Pushed(Button type) const {
//#ifdef USE_DEBUG_CODE
//		if (ImGui::GetIO().WantCaptureMouse or ImGui::IsAnyItemHovered()) {
//			return false;
//		}
//#endif // USE_DEBUG_CODE
	...
	bool Gamepad::LongPush(Button type) const {
//#ifdef USE_DEBUG_CODE
//		if (ImGui::GetIO().WantCaptureMouse or ImGui::IsAnyItemHovered()) {
//			return false;
//		}
//#endif // USE_DEBUG_CODE
	...
	bool Gamepad::Released(Button type) const {
		return !GetButton(type) && GetPreButton(type);
	}

	bool Gamepad::PushAnyKey() const {
//#ifdef USE_DEBUG_CODE
//		if (ImGui::GetIO().WantCaptureMouse or ImGui::IsAnyItemHovered()) {
//			return false;
//		}
//#endif // USE_DEBUG_CODE
	...
	float Gamepad::GetTriger(Triger type, float deadZone) const {
	...
	Vector2 Gamepad::GetStick(Stick type, float deadZone) const {
	...
	Vector2 Gamepad::GetPreStick(Stick type, float deadZone) const
	{
		static constexpr float kNormal = 1.0f / static_cast<float>(SHRT_MAX);
		Vector2 moveStick = Vector2::kZero;
		deadZone = std::clamp(deadZone, 0.0f, 1.0f);
	...
	bool Gamepad::GetButtonStick(Button buttonType, Stick stickType, float deadZone) const {
		bool result = false;
		switch (buttonType)
		{
	...
	bool Gamepad::PushedButtonStick(Button buttonType, Stick stickType, float deadZone) const
	{
		bool result = false;
	...
	bool Gamepad::ReleasedButtonStick(Button buttonType, Stick stickType, float deadZone) const
	{
		bool result = false;
