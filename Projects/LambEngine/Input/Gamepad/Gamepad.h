...
namespace Lamb {
	...
	public:
		...
		bool GetButton(Button type) const;
		...
		/// <param name="type">ボタンタイプ</param>
		/// <returns>1:押された 0:押されてない</returns>
		bool GetPreButton(Button type) const;
		...
		/// <param name="type">ボタンタイプ</param>
		/// <returns>1:押された 0:押されてない</returns>
		bool LongPush(Button type) const;
		...
		/// <param name="type">ボタンタイプ</param>
		/// <returns>1:押された 0:押されてない</returns>
		bool Released(Button type) const;

		/// <summary>
		/// 何かしらのキーやトリガーが押された
		/// </summary>
		/// <returns>押されたらtrue</returns>
		bool PushAnyKey() const;
		...
		float GetTriger(Triger type, float deadZone = 0.3f) const;
		...
		Vector2 GetStick(Stick type, float deadZone = 0.3f) const;
		...
		/// <param name="deadZone">デッドゾーン(0.0f～1.0f)の範囲内なら0.0fを返す</param>
		/// <returns>-1.0f ～ 1.0f の値</returns>
		Vector2 GetPreStick(Stick type, float deadZone = 0.3f) const;
		...
		bool GetButtonStick(Button buttonType, Stick stickType, float deadZone = 0.3f) const;
		/// <summary>
		/// 特定のボタンとスティック入力同期
		/// </summary>
		/// <param name="buttonType">ボタンタイプ(十字ABXYのみ)</param>
		...
		bool PushedButtonStick(Button buttonType, Stick stickType, float deadZone = 0.3f) const;
		...
		bool ReleasedButtonStick(Button buttonType, Stick stickType, float deadZone = 0.3f) const;
